...
export default function Sidebar({ className = "", mobile = false, open = false, onClose }: { className?: string, mobile?: boolean, open?: boolean, onClose?: () => void }) {
  ...
  if (mobile) {
    return (
      <>
        {open && (
          <>
            <div className="fixed inset-0 z-[9998] bg-black bg-opacity-40" onClick={onClose}></div>
              ...
              <div className="border-t border-gray-700 p-4 flex flex-col items-center gap-2 shrink-0">
                {user && (
                  <div className="flex flex-col items-center text-gray-300 text-xs">
                    ...
                    <span className="font-semibold">{user.user_metadata?.name || user.email}</span>
  ...
  // 桌面端
  return (
    <div className={`hidden sm:flex sm:w-64 w-0 flex-col bg-gray-900 ${className} h-full`}>
      {/* 顶部标题固定 */}
      <div className="flex h-16 items-center px-4 shrink-0">
        <h1 className="text-xl font-bold text-white">Mini Linear</h1>
      </div>
